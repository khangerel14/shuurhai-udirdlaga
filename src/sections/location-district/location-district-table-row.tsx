...
import { useChangeStatusDistrictMutation } from 'src/actions/location-district';
...
type Props = Readonly<{
  ...
  index: number;
  refetch: () => void;
  handleRowAction: (rowDialogType: DIALOG_TYPE, row?: IDistrictType) => void;
}>;

export function LocationDistrictTableRow({ row, index, refetch, handleRowAction }: Props) {
  const popover = usePopover();
  const confirmDialog = useBoolean();

  const { loading, executeChangeStatusDistrict } = useChangeStatusDistrictMutation({ refetch });
        ...
        {Object.values(STATUS_ENUM)
          ...
          .map((status) => (
            <MenuItem
              key={status}
              onClick={() => {
                executeChangeStatusDistrict({ rowId: row.id, status });
                popover.onClose();
              }}
            >
    ...
    <ConfirmDialog
      ...
      action={
        <Button
          ...
          variant="contained"
          onClick={() =>
            executeChangeStatusDistrict({ rowId: row.id, status: STATUS_ENUM.DELETED }).then(() => {
