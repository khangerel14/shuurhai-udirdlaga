...
import type { ICallReason } from 'src/types/call-reason';
...
import { EmergencySchema, type EmergencySchemaType } from 'src/schema/emergency-schema';
import {
  useGetCallReasonsFirstQuery,
  useGetCallReasonsThirdQuery,
  useGetCallReasonsSecondQuery,
  useCreateCallEmergencyMutation,
} from 'src/actions/call';
...
export const CallEmergencyForm = ({
  cities,
  dialog,
  address,
  location,
  ...
  const { executeCreateCallEmergency, loading } = useCreateCallEmergencyMutation();
  const methods = useForm<EmergencySchemaType>({
    defaultValues: {
      address: address ?? '',
  ...
  const { first } = useGetCallReasonsFirstQuery();
  const { second } = useGetCallReasonsSecondQuery({ secondId: watch('callReasonFirstId') });
  const { third } = useGetCallReasonsThirdQuery({ thirdId: watch('callReasonSecondId') });
  ...
  useEffect(() => {
    reset({
      ...
      longitude: location?.longitude ?? 0,
      latitude: location?.latitude ?? 0,
      type: DRIVER_TYPE_ENUM.EMERGENCY,
    });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const { message, success } = await executeCreateCallEmergency({
        ...data,
      });
      if (success) {
        reset();
      ...
      <Form methods={methods} onSubmit={onSubmit}>
        <Card sx={{ p: 2, py: 1.5 }}>
          ...
          <Grid container spacing={2} py={1} mb={1}>
            ...
            <Grid size={{ xs: 4 }}>
              <Field.Select name="callReasonFirstId" label="Дуудлагын анхдагч төрөл" size="small">
                {first?.map((item: ICallReason) => (
            ...
            <Grid size={{ xs: 4 }}>
              <Field.Select name="callReasonSecondId" label="Дуудлагын хоёрдогч төрөл" size="small">
                {second?.map((item: ICallReason) => (
                  <MenuItem key={item.id} value={item.id}>
                    {item.name}
                  </MenuItem>
                ))}
            ...
            <Grid size={{ xs: 4 }}>
              <Field.Select name="callReasonThirdId" label="Дуудлагын гуравдагч төрөл" size="small">
                {third?.map((item: ICallReason) => (
                  <MenuItem key={item.id} value={item.id}>
                    {item.name}
                  </MenuItem>
                ))}
