...
import { useChangeVehicleModelStatusMutation } from 'src/actions/vehicle-model';
...
type Props = Readonly<{
  ...
  index: number;
  refetch: () => void;
  handleRowAction: (rowDialogType: DIALOG_TYPE, row?: VehicleModel) => void;
}>;

export function VehicleModelTableRow({ row, index, refetch, handleRowAction }: Props) {
  const popover = usePopover();
  const confirmDialog = useBoolean();

  const { loading, executeChangeStatusVehicleModel } = useChangeVehicleModelStatusMutation({
    refetch,
  });
        ...
        {Object.values(STATUS_ENUM)
          ...
          .map((status) => (
            <MenuItem
              key={status}
              onClick={() => {
                executeChangeStatusVehicleModel({ rowId: row.id, status });
                popover.onClose();
              }}
            >
    ...
    <ConfirmDialog
      ...
      action={
        <Button
          ...
          variant="contained"
          onClick={() =>
            executeChangeStatusVehicleModel({ rowId: row.id, status: STATUS_ENUM.DELETED }).then(
              () => {
  ...
  return (
    <TableRow>
      ...
      <TableCell>{row.vehicleType.name}</TableCell>
      <TableCell>{row.vehicleManufacture.name}</TableCell>
      <TableCell>
