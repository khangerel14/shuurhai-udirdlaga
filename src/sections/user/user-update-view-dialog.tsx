...
import { useUserUpdateMutation } from 'src/actions/user';
...
export function UserUpdateViewDialog({ dialogType, dialog, refetch, currentRow }: Props) {
  const { executeUpdateUserMutation, loading } = useUserUpdateMutation();
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const { message, success } = await executeUpdateUserMutation({
        updateUserId: currentRow?.id ?? '',
