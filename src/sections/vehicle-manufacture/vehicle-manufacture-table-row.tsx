...
import { useChangeStatusVehicleManufacture } from 'src/actions/vehicle-manufacture';
...
type Props = Readonly<{
  ...
  index: number;
  refetch: () => void;
  handleRowAction: (rowDialogType: DIALOG_TYPE, row?: VehicleManufacture) => void;
}>;

export function VehicleManufactureTableRow({ row, index, refetch, handleRowAction }: Props) {
  const popover = usePopover();
  const confirmDialog = useBoolean();

  const { loading, executeChangeStatusVehicleManufacture } = useChangeStatusVehicleManufacture({
    refetch,
  });
        ...
        {Object.values(STATUS_ENUM)
          ...
          .map((status) => (
            <MenuItem
              key={status}
              onClick={() => {
                executeChangeStatusVehicleManufacture({ rowId: row.id, status });
                popover.onClose();
              }}
            >
    ...
    <ConfirmDialog
      ...
      action={
        <Button
          ...
          variant="contained"
          onClick={() =>
            executeChangeStatusVehicleManufacture({
              rowId: row.id,
