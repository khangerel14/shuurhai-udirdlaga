...
import { useChangeStatusCommitteeMutation } from 'src/actions/location-committee';
...
type Props = Readonly<{
  ...
  index: number;
  refetch: () => void;
  handleRowAction: (rowDialogType: DIALOG_TYPE, row?: ICommitteeType) => void;
}>;

export function LocationCommitteeTableRow({ row, index, refetch, handleRowAction }: Props) {
  const popover = usePopover();
  const confirmDialog = useBoolean();

  const { loading, executeChangeStatusCommittee } = useChangeStatusCommitteeMutation({ refetch });
        ...
        {Object.values(STATUS_ENUM)
          ...
          .map((status) => (
            <MenuItem
              key={status}
              onClick={() => {
                executeChangeStatusCommittee({ rowId: row.id, status });
                popover.onClose();
              }}
            >
    ...
    <ConfirmDialog
      ...
      action={
        <Button
          ...
          variant="contained"
          onClick={() =>
            executeChangeStatusCommittee({ rowId: row.id, status: STATUS_ENUM.DELETED }).then(
              () => {
  ...
  return (
    <TableRow>
      ...
      <TableCell>{row.district?.city?.name}</TableCell>
