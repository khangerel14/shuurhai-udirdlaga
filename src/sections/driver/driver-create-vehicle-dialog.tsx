...
const CreateSchema = zod.object({
  userId: zod.string().min(1, 'Хэрэглэгч сонгоно уу!'),
...
type CreateSchemaType = zod.infer<typeof CreateSchema>;
...
export function DriverCreateVehicleDialog({ dialog, refetch, currentRow }: Props) {
  ...
  const defaultValues = useMemo(() => {
    ...
    return {
      ...
      licencePlate: '',
      certificate: 'http://103.50.205.212:4001/auth/jwt/sign-in/',
      color: '',
      importDate,
    };
  }, [currentRow]);

  const methods = useForm<CreateSchemaType>({
    mode: 'all',
    defaultValues,
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={dialog.value} onClose={handleClose}>
      <DialogTitle>{dialogTypeName(DIALOG_TYPE.CREATE, 'Машин')}</DialogTitle>
