...
import { useChangeStatusVehicleType } from 'src/actions/vehicle-type';
...
type Props = Readonly<{
  ...
  index: number;
  refetch: () => void;
  handleRowAction: (rowDialogType: DIALOG_TYPE, row?: VehicleType) => void;
}>;

export function VehicleTypeTableRow({ row, index, refetch, handleRowAction }: Props) {
  const popover = usePopover();
  const confirmDialog = useBoolean();

  const { loading, executeChangeStatusVehicleType } = useChangeStatusVehicleType({ refetch });
        ...
        {Object.values(STATUS_ENUM)
          ...
          .map((status) => (
            <MenuItem
              key={status}
              onClick={() => {
                executeChangeStatusVehicleType({ rowId: row.id, status });
                popover.onClose();
              }}
            >
    ...
    <ConfirmDialog
      ...
      action={
        <Button
          ...
          variant="contained"
          onClick={() =>
            executeChangeStatusVehicleType({ rowId: row.id, status: STATUS_ENUM.DELETED }).then(
              () => {
